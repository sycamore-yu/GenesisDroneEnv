...
def test_shared_contract_matches_spec():
    ...
    assert RACING_CONTRACT["policy_observation_size"] == 40
    assert RACING_CONTRACT["critic_observation_size"] == 41
    assert RACING_CONTRACT["action"] == "ctbr"


def test_ppo_task_exposes_observation_groups_timeouts_and_discount():
    _ensure_genesis()
    environment = RaceEnv(RaceEnvConfig(horizon=1, max_episode_steps=3, enable_gate_contact=False), 2, requires_grad=False)
    with (Path(__file__).resolve().parents[1] / "config" / "race" / "ppo.yaml").open() as file:
        train_config = yaml.safe_load(file)
    assert train_config["algorithm"]["gamma"] == 0.999
    assert train_config["algorithm"]["lam"] == 0.95
    task = RaceTask(environment, train_config)
    observation = task.get_observations()
    assert observation["policy"].shape == (2, 40)
    assert observation["critic"].shape == (2, 41)
